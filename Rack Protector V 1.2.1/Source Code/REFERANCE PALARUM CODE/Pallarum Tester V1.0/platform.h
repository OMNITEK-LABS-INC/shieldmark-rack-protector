...
#define platformDelay(t)                              __delay_ms(t)                                                       /*!< Performs a delay for the given time (ms)    */

#define platformGetSysTick()                          HAL_GetTick()                                                      /*!< Get System Tick ( 1 tick = 1 ms)            */

#if !(ST25R95_INTERFACE_UART) /* ST25R95_INTERFACE_SPI */
#define platformSpiSelect()                           LATBbits.LATB0 = 0            /*!< SPI SS\CS: Chip|Slave Select                */
#define platformSpiDeselect()                         LATBbits.LATB0 = 1              /*!< SPI SS\CS: Chip|Slave Deselect              */
//#define platformSpiTxRx(txBuf, rxBuf, len)            spi_transceive_buffer_wait((txBuf), (rxBuf), (len))   /*!< SPI transceive                              */
